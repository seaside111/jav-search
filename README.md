...
- 🎬 **自动刮削归档**：监控下载目录，完成后写 Emby/Kodi NFO + poster/fanart，重命名为番号，按 `YYYYMM/番号/` 归档；只在明确分段时生成 CD 后缀，并避免把同内容的不同清晰度版本误判为分段；图片缺失时有限次逐源补查、命中即停
...
> 当前正式版本为 `v1.4.6.3`，也可以将镜像标签 `:latest` 换成固定版本标签 `:V1.4.6.3`。
