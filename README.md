...
- 🌐 **多数据源**：JavBus + JavDB 为主，可选 AVSOX（无码）/ AVMOO（旧片）/ FC2（FC2-PPV 素人无码），并新增默认关闭的 JAV321 与 DMM/FANZA 图片补全源
...
- 🎬 **自动刮削归档**：监控下载目录，完成后写 Emby/Kodi NFO + poster/fanart；可保留原视频名，自定义番号/标题/演员文件夹名，并可用完整横向封套生成 fanart、从右侧正面裁出 poster；图片缺失时有限次逐源补查、命中即停
- 👤 **演员头像增量补全**：首轮优先 JavBus/AVSOX，成功即停；缺失演员进入 JavDB 低频目录队列，命中后更新全局缓存、关联影片 `actors/` 和 Emby Person 主图
...
│       ├── jav321.py           # JAV321 可选图片/元数据补全源
│       ├── dmm.py              # DMM/FANZA 官方 API 可选图片/元数据补全源
...
> **V1.4.4 FC2 专项优化**：① FC2「最新片源」改为**优先用 sukebei 发现番号**（种子站按 id 倒序＝最新、直连不过盾、最快），并**统一按 FC2-PPV 编号降序**取最新；② 新增**后台串行预抓 MissAV**（直连不过盾、低负担），慢慢灌入最新片的真标题/封面/样品图；③ FC2 详情改为 **MissAV-only**，点开即出、不碰 FlareSolverr；④ 设置页新增相关开关。详见 [`更新说明.md`](更新说明.md#v14642026-06-10)。

> **V1.4.3 首页增强 + FC2 数据源**：① 顶部新增**版本号 + 更新检测**；② 首页最新片源**各来源并行、边抓边显示**；③ 列表上方**数据源切换标签**；④ 新增 **FC2-PPV 数据源**（fc2ppvdb.com，走 FlareSolverr），番号 `FC2-PPV-1234567` 等自动识别。详见 [`更新说明.md`](更新说明.md#v1432026-06-07)。

> **V1.4.2 优化（JavDB 专项）**：① JavDB 反爬增强（Cookie + 完整请求头 + 重试）；② 可选 FlareSolverr 过盾；③ 设置页新增「JavDB 连通测试」；④ JavDB 详情扩充磁力链、样品图、评分人数。详见 [`更新说明.md`](更新说明.md#v1422026-06-05)。

> **V1.4.1 修复**：① 推送下载时记录搜索结果的准确番号，刮削时优先采用；② 服务地址填 `http://localhost` 时种子直链改由后端代取，修复外网点击 `.torrent` 打不开。详见 [`更新说明.md`](更新说明.md#v1412026-06-04)。
