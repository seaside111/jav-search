...
- 📋 **任务状态管理**：按源文件独立展示下载、刮削、归档和失败状态，并支持按番号、文件名、路径或错误搜索。
- 🎬 **自动刮削与归档**：监控下载目录，识别单片、多分段及字幕文件，生成 Emby/Kodi 兼容的 NFO、`poster.jpg`、`fanart.jpg`，并按自定义规则整理媒体文件。
- 👤 **演员与媒体库同步**：下载并缓存本地演员头像，可选择是否在每部影片目录保留 `actors` 文件夹，并可将本地图片同步为 Emby 演员主图；支持字幕封面角标及历史归档修复。
...
> 当前正式版本为 `v1.4.6.25`，也可以将镜像标签 `:latest` 换成固定版本标签 `:v1.4.6.25`。
