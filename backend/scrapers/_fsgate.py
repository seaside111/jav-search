...
# ──────────────────────────────────────────────
# 优先级（数值越小越先执行）：详情点击 < 用户搜索 < 首页最新 < 后台批量刮削
# 用途：FlareSolverr 是单实例、串行处理，批量任务排进队列后，用户点详情若按 FIFO
# 就得排到最后。用优先级闸让「详情」这类交互请求插到批量任务前面（仅能重排「等待中」
# 的请求，无法抢占正在执行的那一个——单浏览器一次只跑一个）。
# ──────────────────────────────────────────────
PRIO_DETAIL = 0
PRIO_SEARCH = 5
PRIO_LATEST = 10
...
class _PriorityLock:
    """串行闸 + 优先级唤醒：语义等价 Semaphore(1)，但释放时优先唤醒优先级最高
    （priority 数值最小）的等待者；同优先级按到达顺序（FIFO，用单调序号打破平手）。
    支持 await acquire(priority) / release()，并妥善处理等待期间被取消（客户端断开）。"""
    ...
    async def acquire(self, priority: int = PRIO_LATEST) -> None:
        # 无人持有且无人等待 → 直接拿锁
        ...
        try:
            ...
            # 等待被取消：若已被选中（拿到锁）须转交下一个，否则把自己从堆里摘掉
            if fut.done() and not fut.cancelled():
                self._wake_next()
            else:
                self._discard(fut)
            raise
        self._locked = True               # 被 release 选中唤醒，锁已转交给我们
    ...
    def _wake_next(self) -> None:
        while self._heap:
            _prio, _seq, fut = heapq.heappop(self._heap)
            if not fut.done():
                fut.set_result(None)      # 选中下一个；_locked 保持 True，由其接管
                return
        self._locked = False              # 无等待者，真正释放

    def _discard(self, target) -> None:
        self._heap = [(p, s, f) for (p, s, f) in self._heap if f is not target]
        heapq.heapify(self._heap)


# ── 三层保护的全局状态（进程级，跨模块共享）──
GATE = _PriorityLock()             # 串行 + 优先级：一次只发一个请求，详情可插队
_MAX_WAITERS = 16                  # 排队上限（交互类）：超过直接快失败，杜绝无限堆积压垮实例
_MAX_WAITERS_BULK = 12             # 批量类（首页最新/刮削）上限：留 4 个槽给交互类，繁忙时详情不被挤掉
...
async def flaresolverr_request(url: str, flaresolverr_url: str, proxy: Optional[str] = None,
    ...
    依次经过：熔断检查 → 背压检查 → 全局串行+优先级闸 → 最小间隔 → 多候选尝试。
    任何一层快失败都返回可读错误，绝不长时间挂起或继续压垮实例。
    priority：数值越小越先执行（见 PRIO_*）。详情点击传 PRIO_DETAIL 可插到批量任务前面。
    """
    ...
    #    批量类（首页最新/刮削，priority>=PRIO_LATEST）用更低的阈值，给交互类（详情/搜索）
    #    预留几个槽——这样实例繁忙、批量把队列占满时，用户点详情不会被「排队已满」挡掉。
    limit = _MAX_WAITERS_BULK if priority >= PRIO_LATEST else _MAX_WAITERS
    if _waiters >= limit:
        return "", 0, (f"flaresolverr 异常: 排队请求已达上限（{limit}），实例处理不过来，"
                       "已快速放弃本次以保护实例。请稍后再试，或降低并发/最新片源条数。")

    _waiters += 1
    try:
        # 3) 全局串行 + 优先级：整轮候选尝试在同一把闸内，一次只向 FlareSolverr 发一个请求；
        #    释放时优先唤醒高优先级（详情）等待者。
