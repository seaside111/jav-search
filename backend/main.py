...
APP_VERSION = (os.environ.get("JAV_SEARCH_VERSION") or "1.4.6.14").strip().lstrip("vV")
...
class ConfigUpdateRequest(BaseModel):
    ...
    magnet_delete_completed: Optional[bool] = None    # 磁力链下完后自动删种(保留文件)
    # V1.4 qBittorrent
...
async def fetch_image_cached(url: str) -> Optional[tuple[bytes, str]]:
    ...
    返回 (bytes, content-type)；彻底失败返回 None。

    /api/img（首页/详情封面、样品图）与刮削写封面共用本函数，确保：
      ① 刮削直接复用首页浏览时已缓存的封面（命中即零上游请求，不再按地址重下）；
      ② 缓存未命中需回源时，也走同一套防盗链抓取逻辑（修复 FC2 等封面因 Referer 错误下载失败）。"""
    ...
    if disk:
        _img_mem_put(url, disk[0], disk[1])   # 回填内存，下次走一级
        return disk
...
async def api_details(req: DetailsRequest):
    ...
    详情缓存在 scrapers.enrich 内部统一处理（按 url 缓存，前台预抓/点开/刮削回源共享），
    故命中缓存的条目直接返回、不重复抓取（尤其 JavDB 经 FlareSolverr 的磁力/样品图）。
    """
    if not req.items:
