...
DEFAULT_CONFIG = {
    ...
    # ── V1.5.1：HD 发种阈值（等大文件再发）──
    # 场景：新番号上架时,常是 720p 小文件种子先出现、高清大文件种子后出现。若一找到就发,会发出小文件版。
    # 故设「HD 发种阈值」：找到的最佳种子体积低于此 → 视为「仅小文件先出」,暂不发种,留在蓄种池等后续轮次
    # 出现大文件;等满 size_wait_rounds 轮仍只有小文件,才按设置放行发布当前最佳(受 min_size 绝对下限约束)。
    # 与 min_size 的区别：min_size 是「绝对下限」(堵样片,低于即不选);publish_min_size 是「优选阈值」(低于则等,不是不选)。
    "autopilot_publish_min_size_mb": 1024,   # HD 发种阈值(MB):最佳种子体积低于此则暂不发、留池等大文件;0=不启用(找到即发)
    "autopilot_size_wait_rounds": 3,         # 等大文件的最多轮询轮数:仅小文件时最多等这么多轮,仍无大文件则按设置发布当前最佳;0=不等(找到即发)
