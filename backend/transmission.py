...
async def add_torrent(
    ...
    async with httpx.AsyncClient(timeout=timeout, follow_redirects=True) as client:
        ...
        # 常规 .torrent（非磁力）也补公共 tracker，与磁力统一。官方 PT 种子 public_trackers=False 跳过。
        was_magnet = bool(download_url and download_url.lower().startswith("magnet:"))
        if public_trackers and ih and not was_magnet:
            from qbittorrent import _current_trackers
            trs = _current_trackers()
            if trs:
                await _rpc(client, url, _auth(username, password), sid,
                           "torrent-set", {"ids": [ih], "trackerAdd": trs})
